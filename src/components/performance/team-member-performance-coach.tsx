...
import type { allTeamMembers } from '@/lib/users';

const COMPANY_KPI = 80; // 80% completion rate as the target

type Member = (typeof allTeamMembers)[0];

interface TeamMemberPerformanceCoachProps {
    member: Member;
}
...
export function TeamMemberPerformanceCoach({ member }: TeamMemberPerformanceCoachProps) {
  ...
  // In a real app, you'd filter tasks for the specific member.
  // For now, we use a subset of global tasks for demonstration.
  const memberTasks = tasks.slice(0, Math.floor(tasks.length * 0.7));
