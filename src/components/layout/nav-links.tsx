...
export function NavLinks({ isMobile = false }: { isMobile?: boolean }) {
  ...
  const renderLink = (href: string, icon: React.ElementType, label: string, notificationKey?: string) => {
    ...
    return (
        ...
        href={href}
        className={cn(
            'flex items-center gap-3 rounded-lg px-3 py-2 text-primary transition-all shadow-menu-item shadow-primary/20 hover:bg-primary hover:text-primary-foreground',
